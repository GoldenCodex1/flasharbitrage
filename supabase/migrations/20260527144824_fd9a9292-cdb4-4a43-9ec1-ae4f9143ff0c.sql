...
-- 1) Add network column to trades
ALTER TABLE public.trades ADD COLUMN IF NOT EXISTS network text;

-- Backfill any trades missing a network with a random realistic chain
UPDATE public.trades
SET network = (ARRAY['TRC20','ERC20','BEP20','BTC','SOL'])[1 + floor(random()*5)::int]
WHERE network IS NULL;

-- 2) Network-aware wallet address generator
...
DECLARE
  ...
  _src text;
BEGIN
  -- Generate a long random hex pool from chained md5s
  ...
  IF _net IN ('ERC20','BEP20','ETH','BSC') THEN
    -- 0x + 40 hex chars (Ethereum / BSC style)
    RETURN '0x' || substr(_hex, 1, 40);

  ELSIF _net IN ('TRC20','TRON','TRX') THEN
    -- TRON addresses: 'T' + 33 base58 chars (total length 34)
    _src := _b58_chars;
    _len := 33;
    _out := 'T';
  ...
  ELSIF _net IN ('BTC','BITCOIN') THEN
    -- Bech32 bc1q + 38 chars from bech32 alphabet (total 42)
    _src := _b32_chars;
    _len := 38;
    _out := 'bc1q';
    FOR _i IN 1.._len LOOP
  ...
  ELSIF _net IN ('SOL','SOLANA') THEN
    -- Solana addresses: 43-44 base58 chars
    _src := _b58_chars;
    _len := 44;
    FOR _i IN 1.._len LOOP
      _out := _out || substr(_src, 1 + (('x' || substr(_hex, 1 + ((_i-1)*2) % 60, 2))::bit(8)::int) % length(_src), 1);
  ...
  ELSE
    -- Fallback ERC20-style
    RETURN '0x' || substr(_hex, 1, 40);
  END IF;
END;
$$;

GRANT EXECUTE ON FUNCTION public.gen_wallet_address_for_network(text, text) TO authenticated, service_role;

-- 3) Update join_trade to use network-aware addresses
...
BEGIN
  ...
  _net := COALESCE(_trade.network, (ARRAY['TRC20','ERC20','BEP20','BTC','SOL'])[1 + floor(random()*5)::int]);

  -- Persist network if it was missing
  IF _trade.network IS NULL THEN
